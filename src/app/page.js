...
export default function Home() {
  ...
  useEffect(() => {
    if (state?.status != null && state?.status !== 200) {
      ...
      Objective: \n ${state.body.objective.map((item) => `- ${item.name} `).join("\n ")}
      
      Key Features: \n ${state?.body.key_features.map((item) => `- ${item.name}: ${item.detail}`).join("\n ")}
      {}
      User Stories: \n ${state?.body.user_stories.map((item) => `- ${item.name} `).join("\n ")}
      `);
    }
  }, [state]);
    ...
    <main className="w-full px-4 md:px-6">
      ...
      <div className="max-w-3xl bg-red-40 mx-auto pt-8 md:pt-24 pb-6 lg:mt-52">
        ...
        <form className="flex flex-col items-center" action={formAction}>
          <input name="features" type="text" placeholder="Type here" className="input input-bordered w-full max-w-3xl rounded-full" />
          <button
            type="submit"
          ...
          <div className="mx-6">
            ...
            <div className="flex flex-row justify-end gap-4 w-full py-4">
              <CopyToClipboard text={brief}>
                <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-2 rounded flex-grow-0">Copy</button>
              </CopyToClipboard>
              <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-2 rounded flex-grow-0" onClick={handleSave}>
                Save
              </button>
            </div>
          </div>
